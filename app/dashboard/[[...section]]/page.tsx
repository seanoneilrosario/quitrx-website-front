...
export default async function DashboardPage({ params, searchParams }: Props) {
  ...
  return <div className={styles.shell}><aside><Link className={styles.logo} href="/"><span>Q</span><strong>QuitRX</strong></Link><nav>{nav.map((item) => <Link className={active === item.href ? styles.active : ""} key={item.href} href={item.href}><i>{item.icon}</i>{item.label}</Link>)}</nav><div className={styles.profile}><span>ST</span><div><strong>Staff account</strong><small>Operations</small></div></div></aside><main><div className={styles.mobileTop}><Link className={styles.logo} href="/"><span>Q</span><strong>QuitRX</strong></Link><nav>{nav.map((item) => <Link key={item.href} href={item.href}>{item.label}</Link>)}</nav></div>{content}</main></div>;
}
